...
class Image extends Model
{
	...
	public function url($size = null)
    {
        ...
        return preg_replace("#\b{$locale}/\b#","",$url);
	...
	protected function pathFromSize($size = null)
	{
		if (null === $size) {
			...
			return (isset($this->path))?$this->path:Config::get('laravel-image::storage_base_path') . '/' . $this->filename;
		}
...
//		$image->filename = md5(Str::random(32)) . '.' . $resizer->extension();
		$image->filename = $resizer->width().'x'.$resizer->height().'-'.array_get($data, 'original_filename');
		...
		$destpath = $image->pathFromSize();
		Storage::upload($path, $destpath);
