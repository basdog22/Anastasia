...
if ( ! function_exists('url'))
{
	...
	{
        if(isset($parameters['no_locale'])){
            unset($parameters['no_locale']);
            $no_locale=true;
        }else{
            $no_locale = false;
        }
        if((get_default_locale()!=get_current_locale()) && !starts_with($path,'http') && !defined('IN_ADMIN_AREA') && !$no_locale){
            $path = trim($path,"/");
            $path = get_current_locale().'/'.$path;
        }
