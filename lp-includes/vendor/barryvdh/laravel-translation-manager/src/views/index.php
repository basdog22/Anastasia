    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                if($(this).val().length){
                    window.location.href = '<?= url('backend/translations/index') ?>/'+$(this).val();
                }
...
<div class="col-md-12">
    <div class="box">
        <div class="box-header">
            <div class="box-name">
                <span><?= Lang::get('strings.translation_manager')?></span>
            </div>
            <div class="box-icons col-md-6">
                <?php if(!isset($group)) : ?>
                    <form class="form-inline col-md-8 form-import" method="POST" action="<?= url('backend/translations/import') ?>" data-remote="true" role="form">
                        <select name="replace" class="form-control no-select2">
                            <option value="0"><?= Lang::get('strings.append_translations') ?></option>
                            <option value="1"><?= Lang::get('strings.replace_translations')?></option>
                        </select>
                        <button type="submit" class="btn btn-success"  data-disable-with="Loading.."><?= Lang::get('strings.import_groups')?></button>
                    </form>
                    <form class="form-inline col-md-4 form-find pull-right" method="POST" action="<?= url('backend/translations/find') ?>" data-remote="true" role="form" data-confirm="<?= Lang::get('strings.scan_question')?>">
                        <button type="submit" class="btn btn-info" data-disable-with="Searching.." ><?= Lang::get('strings.find_translations')?></button>
                    </form>
                <?php endif; ?>
                <?php if(isset($group)) : ?>
                    <form class="form-inline form-publish pull-right" method="POST" action="<?= url('backend/translations/publish', $group) ?>" data-remote="true" role="form" data-confirm="<?= Lang::get('strings.publish_question',['group'=>$group])?>">
                        <button type="submit" class="btn btn-info" data-disable-with="Publishing.." ><?= Lang::get('strings.publish_translations')?></button>
                    </form>
                <?php endif; ?>
            </div>
        </div>
        <div class="box-content">
            <p><?= Lang::get('strings.warning_about_translations')?></p>
            <div class="alert alert-success success-import" style="display:none;">
                <p><?= Lang::get('strings.done_importing')?> <?= Lang::get('strings.reload_page_groups')?></p>
            </div>
            <div class="alert alert-success success-find" style="display:none;">
                <p><?= Lang::get('strings.done_searching')?></p>
            </div>
            <div class="alert alert-success success-publish" style="display:none;">
                <p><?= Lang::get('strings.done_publishing')?> '<?= $group ?>'!</p>
            </div>
            ...
            <form role="form">
                <div class="form-group">
                    <?= Form::select('group', $groups, $group, array('class'=>'no-select2 form-control group-select')) ?>
                </div>
            </form>
            <?php if($group): ?>
                <form action="<?= url('backend/translations/add', array($group)) ?>" method="POST"  role="form">
                ...
                <h4><?= Lang::get('strings.total')?>: <?= $numTranslations ?>, <?= Lang::get('strings.changed')?>: <?= $numChanged ?></h4>
                <table class="table">
                    <thead>
                    <tr>
                        <th width="15%">Key</th>
                    ...
                    <?php foreach($translations as $key => $translation): ?>
                        <tr id="<?= $key ?>">
                            ...
                            <?php if($deleteEnabled): ?>
                                <td>
                                    <a href="<?= url('backend/translations/delete', [$group, $key]) ?>" data-confirm="<?= Lang::get('strings.delete_question')?>" ><span class="fa fa-trash"></span></a>
                                </td>
                            <?php endif; ?>
                        </tr>
                    <?php endforeach; ?>
            ...
            <?php else: ?>
                <p><?= Lang::get('strings.translations_bottom_warn')?></p>
